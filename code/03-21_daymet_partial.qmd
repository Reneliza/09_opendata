...
```
geom_density - special type of geom that calculates the y-axis for you.

```{r}
states <- us_states() %>%
  filter(!state_abbr %in% c("PR", "AK","HI")) # ! exclamation point means the opp of the argument
...
```
sf object has a geometry column that carries the spatial objects
...
*API - connecting point between the database and the user
...
We usually want each column to be a variable
```{r}
daymet_one %>%
...
```
How many rows above? Why?  365 coz each row is for a given day of the year;

pivot_wider placed/transformed the variables into columns
...
For that, let's use the **pmap()** family of functions from the purrr package.  
